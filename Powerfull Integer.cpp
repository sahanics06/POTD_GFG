...
public:
    ...
    int powerfullInteger(int n,vector<vector<int>> &intervals,int k){
        ...
        map<int, int>m;
        int ans=-1, tmp=0;
        for(auto x:intervals)   // For every start index we are incrementing its value from 0 and storing in map so that it is stored in ordered way.
        {                       // For ending index we decrement value in its next value.
            m[x[0]]++;
            m[x[1]+1]--;
        }
        for(auto x:m)
