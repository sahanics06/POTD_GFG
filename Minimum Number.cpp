...
//  Solution using gcd and writing own gcd. Gcd is calculated using subtraction. Gives TLE.
...
public:
    ...
    int minimumNumber(int n,vector<int> &arr){
        ...
        for(int i=1; i<n; i++)
        {
            ans=gcd(max(ans, arr[i]), min(ans, arr[i]));
        }
        return ans;
    }
};

// Solution using gcd and writing own gcd. Gcd is calculated using modulo. Solution accepted.
class Solution{
public:
    int gcd(int a, int b)
    {
